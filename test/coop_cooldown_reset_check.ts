...
(async()=>{
  ...
  try{
    ...
    await c.send('Page.addScriptToEvaluateOnNewDocument',{source:`try{localStorage.setItem('magic_web_game_save_v1',${JSON.stringify(JSON.stringify(save))});localStorage.setItem('madoken_sound_v2',JSON.stringify({bgmVolume:0,sfxVolume:0,muted:true}))}catch{}`});
