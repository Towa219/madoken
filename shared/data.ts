...
// ===== ステージをクリアしてから次へ進むまでの間 =====
//
// ★ サーバーとクライアントが同じ数字を見ること。
//   ボスを倒すとコメント弾幕(src/danmaku.ts)が流れる。ここが短いと
//   まだコメントが流れている最中に次のステージが始まり、読む余裕が無い。
//   弾幕はこの秒数のあいだに出し切るよう、逆算して組み立てている。
//
// ★ 変えるのはここだけでよい。弾幕の撒き方も次へ進む間も、
//   両方この数字から決まる。
export const STAGE_NEXT_SEC = 4;        // ふつうのステージ
export const BOSS_CELEBRATE_SEC = 11;   // ボスを倒した後(弾幕を読み切る間)
...
// ★ 端末はサーバーより「ずっと長く」粘ること。同じ90秒にしてはいけない。
//
//   サーバーの90秒は「サーバーが切断に気づいた時」から始まる。ところが
//   気づくのは端末よりかなり遅い。回線が片側だけ死ぬと、サーバーからは
//   接続が生きたままに見え、席は埋まったままになる。その間の復帰要求は
//   全部弾かれる。
//
//   2026-08-18 の実測:
...
//   端末は「席が空く5秒前」に諦めていた。90秒の試行は1回も通らない。
//
//   そこで端末側は、サーバーが気づくまでの遅れ + 席を空けて待つ90秒を
//   見込んだ長さにする。長く粘っても困るのは待たされることだけで、
//   途中でやめたければ退出すればよい。取り返しがつかないのは
//   「あと少しで戻れたのに諦める」ほうなので、長い側へ倒す。
export const RECONNECT_CLIENT_SEC = 240;
export const RECONNECT_TRIES =
  Math.ceil((RECONNECT_CLIENT_SEC * 1000) / RECONNECT_WAIT_MS);
...
// 本番の抽選を始めるかどうか。
//
// false にすると「お試し」に戻る。引くと演出と結果は出るが、チケットは
// 減らず、魔法も研究Pも受け取れない。見た目や確率を確かめたい時に使う。
// 画面の断り書き・説明書・テストの期待値はここを見て切り替わるので、
// 直すのはこの1行だけでよい。
export const GACHA_LIVE = true;
