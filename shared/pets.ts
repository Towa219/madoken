...
// 卵を温められる間隔。朝と夜の2回、が目安。
//
// ★ きっかり24時間や12時間にしてはいけない。「昨日と同じ時刻より
//   1分早い」だけで断られ、触れる時刻が毎日じわじわ後ろへずれていく。
//   少し短くして、だいたい同じ頃に触れば進むようにする。
//
// ★ 20時間から11時間へ縮めた(2026-08-11)。
//   20時間だと初めての1羽が孵るまで2.5日(カラスは3.3日)かかっていた。
//   待っている間ペットの画面には「まだ温められません」しか出ず、
//   この機能のいちばんの見せ場である孵化の演出に辿り着く前に
//   離れてしまう。ブラウザで遊ぶものとして2.5日は長すぎた。
//   11時間なら1.4日で孵り、朝と夜で自然に2回触れる。
//
//   これ以上は縮めないこと。半日で孵るようにすると育てる手応えが消え、
//   成鳥でいられる15〜30日という寿命とも釣り合わなくなる。
export const WARM_INTERVAL_MS = 11 * 60 * 60 * 1000;
...
//
// ★ 代わりに、孵化には鳥の枠の空きが要る(下の whyCannotHatch)。
//   分けた以上これが無いと、卵をためこんでから一気に孵して
//   鳥の上限を越えられてしまう。卵の枠を増やすほどここが効く。
// ★ どちらも交配所へ預けている間は数に入れない ― 預ける動機を作るため。
export const MAX_PETS = 6;   // 鳥(雛・成鳥・老鳥)
// ★ 卵は 6 → 10 に増やした(2026-08-21)。届く卵を取り逃しにくくするため。
//   鳥の枠(MAX_PETS)は据え置き。増やすのは受け取り口だけで、
//   同時に育てられる数は変えない。
export const MAX_EGGS = 10;  // 卵・巣
...
// 1羽のカードに「〇〇と交配」のボタンを何個出すかを決める。
//
// ★ 組める相手だけを出す(2026-08-21)。以前は相手を全員並べて、
//   組めない相手のボタンを押せない状態にしていた。だが相手は
//   「自分の手持ち + 交配所に預けられている全員」なので、預かりが
//   増えるほど押せないボタンが積み上がり、押せる相手が埋もれた。
//   ♂♀が合わないだけで必ず半分は脱落するので、なおさら多い。
//
// ★ 1羽も出せない時は、黙って消さずに理由を返す。何も出ないと
//   「壊れている」と読まれるし、説明書にも「自分の鳥のカードにある
//   〇〇と交配を押す」と書いてあるので、探して見つからない人が出る。
//
//   理由が全員ぶん同じなら、それはこの鳥自身の事情(休んでいる・
//   もう産めない など)なので、そのまま返せば知りたいことが書いてある。
//   バラバラなら相手それぞれの事情なので、まとめて一言にする。
//
// ★ ここで canBreed の判定を書き写さないこと。写すと必ずずれる。
export function breedOptions(
  pet: Pet, partners: Pet[], now: number,
): { 相手: Pet[]; 案内: string | null } {
  const 相手: Pet[] = [];
  const 断られた: string[] = [];
  for (const p of partners) {
    if (p.id === pet.id) continue;
    const reason = canBreed(pet, p, now);
    if (reason) 断られた.push(reason);
    else 相手.push(p);
  }
  if (相手.length > 0 || 断られた.length === 0) return { 相手, 案内: null };
  const 同じ = 断られた.every(r => r === 断られた[0]);
  return { 相手, 案内: 同じ ? 断られた[0] : '今は交配できる相手がいません。' };
}
